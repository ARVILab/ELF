...
struct WinCount {
 public:
	...
	std::string info() const {
		...
		const float wr = n_done_ == 0 ? 0.0 : static_cast<float>(n_win_) / n_done_;
...
class BatchRequest {
 public:
	...
	std::string stuck_info() const {
		...
		if (stucks_.size() > 0) {
			ss << "\n#stucks_: " << stucks_.size() << ", " << stucks_[0];
		}
		if (nonstuck_zero_.size() > 0) {
			auto it = nonstuck_zero_.begin();
			ss 	<< "\n#nonstuck_zero: " << nonstuck_zero_.size() 
					<< ", " << it->first
				 	<< ", dl: " << it->second;
		}
		return ss.str();
	...
	std::string stuck_detail_info() const {
		...
		if (nonstuck_zero_.size() > 0) {
			ss 	<< ", #nonstuck_zero: " << nonstuck_zero_.size() 
					<< "[";
...
class Pick {
 ...
 private:
	const size_t	num_request_;
	const size_t	max_request_per_layer_;
