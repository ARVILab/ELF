...
class GameFeature {
 public:
  ...
  static void extractOfflineAction(
    ...
    const size_t move_to = s._state.getPly() - 1;
    for (int i = 0; i < s._game_options.num_future_actions; ++i) {
  ...
  void registerExtractor(int batchsize, elf::Extractor& e) {
    ...
    e.addField<int64_t>("offline_a")
        .addExtents(batchsize, {batchsize, game_options_.num_future_actions});
    e.addField<float>({
      "V", 
      "winner", 
      "predicted_value"}).addExtent(batchsize);
  ...
  std::map<std::string, int> getParams() const {    
    return std::map<std::string, int>{
        {"num_action", TOTAL_NUM_ACTIONS},
        {"board_size", 8},
        {"num_future_actions", 1},
        {"num_planes", num_plane_}
    };
  }
