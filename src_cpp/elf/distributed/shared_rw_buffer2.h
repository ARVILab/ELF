...
// Класс отвечающий за обмен данными между клиентом и сервером на С++
// при принятии сообщений он проводит их оброботку с помощью фукций
// которые мы передаем в качестве аргументов:
//			ProcessFunc proc_func - обрабатывает полученную информацию от клиента,
//			ReplyFunc replier = nullptr - после обработки онформации вызывается этот
// 					метод, если мы хотим отправить клиенту информацию обратно
// 					(например какую модель использовать для дальнейшей генерации батчей).
//			StartFunc start_func = nullptr - используется перед стартом, к примеру
// 					нам нужно подождать какое-то время, перед тем как клиент сгенерирует
// 					нам необходимый батч для обучения
// 			
class Reader {
 public:
	...
	// тут передаем наши функции и сразу запускаем нашу StartFunc start_func
	// а уже после выполняется метод, который вызывает методы
	// 		ProcessFunc proc_func и ReplyFunc replier
	void startReceiving(
	...
	void threaded_receive_msg(
		...
		while (!done_.load()) {
			...
			if (title == "ctrl") {
				client_size_++;
				logger_->info(
						"Ctrl from client {} client_size={}; msg={}",
						identity,
						client_size_,
						msg);
